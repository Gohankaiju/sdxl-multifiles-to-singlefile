...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="モデルと保存パスを処理")
    parser.add_argument("--model_path", type=str, help="モデルフォルダのパス")
    parser.add_argument("--save_path", type=str, default="./outputs", help="変換されたモデルを保存するパス")
    args = parser.parse_args()

    if args.model_path is None:
        print("警告: --model_path が指定されていません。")
        print("使用方法: python script.py --model_path <モデルフォルダのパス> [--save_path <保存先パス>]")
        sys.exit(1)

    model_path = args.model_path
    save_dir = args.save_path
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f"{os.path.basename(model_path)}.safetensors")
    ...
    state_dict = convert_diffusers_unet_state_dict_to_sdxl(pipe.unet.state_dict())
    with init_empty_weights():
        unet = sdxl_original_unet.SdxlUNet2DConditionModel()

    _load_state_dict_on_device(unet, state_dict, device="cuda", dtype=torch.float16)
    ...
    print(f"モデルの変換が完了しました。保存先: {save_path}")